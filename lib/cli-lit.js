...
function installDependencies(outputDir, dependencies) {
  ...
  const cliLitModulesDir = path.join(__dirname, '../node_modules');
    ...
    .then(() => mkdirp.sync(cliModulesDir))
    .then(() => mergedirs(originalModulesDir, cliModulesDir, 'overwrite'))
    .then(() => fs.existsSync(cliLitModulesDir))
    .then((shouldCopyCliDep) => shouldCopyCliDep ? mergedirs(cliLitModulesDir, cliModulesDir, 'overwrite') : null)
